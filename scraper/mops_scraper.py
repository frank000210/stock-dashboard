...
Config (env vars):
  ...
  STOCK_IDS     — Comma-separated stock IDs, or "ALL" to save all companies
  SCHEDULE_HOUR — Hour (0-23) in UTC to run daily (default 9 = 17:00 Taiwan)
  TZ_OFFSET     — Timezone offset in hours from UTC (default 8 for Taiwan)
...
_stock_ids_raw = os.getenv("STOCK_IDS", "ALL").strip()
SAVE_ALL = _stock_ids_raw.upper() == "ALL"
STOCK_IDS = [] if SAVE_ALL else [s.strip() for s in _stock_ids_raw.split(",") if s.strip()]
# Schedule: default 9 UTC = 17:00 Taiwan time (UTC+8)
...
def should_include(sid: str, stock_set: set) -> bool:
    """Check if a stock should be saved. Always True when SAVE_ALL mode."""
    if SAVE_ALL:
        return True
    return sid in stock_set


# --------------- Monthly Revenue (月營收) ---------------
def scrape_monthly_revenue(db):
    """
    ...
    for row in data:
        ...
        if not sid or not should_include(sid, stock_set):
            continue

        period_str = str(row.get("資料年月", "")).strip()  # e.g. "11402"
    ...
    if ops:
        ...
        log.info(f"Revenue: {count} stocks processed, upserted={result.upserted_count}, modified={result.modified_count}")
    else:
        log.warning("No revenue records to save")


# --------------- Quarterly Financial (EPS + 每股淨值) ---------------
def scrape_financial(db):
    """
    ...
    for suffix in INDUSTRY_SUFFIXES:
        ...
        for row in data:
            sid = str(row.get("公司代號", "")).strip()
            if not sid or not should_include(sid, stock_set) or sid in eps_found:
                continue
    ...
    log.info(f"EPS: found {len(eps_found)} stocks")
    ...
    for suffix in INDUSTRY_SUFFIXES:
        ...
        for row in data:
            sid = str(row.get("公司代號", "")).strip()
            if not sid or not should_include(sid, stock_set) or sid in nav_found:
                continue
    ...
    log.info(f"NAV: found {len(nav_found)} stocks")

    if ops:
        result = db["financial"].bulk_write(ops)
        log.info(f"Financial: {len(ops)} ops, upserted={result.upserted_count}, modified={result.modified_count}")
    else:
        log.warning("No financial records to save")


# --------------- Orchestration ---------------
def run_all(db):
    mode = "ALL stocks" if SAVE_ALL else f"{len(STOCK_IDS)} stocks: {STOCK_IDS[:10]}"
    log.info(f"Starting scrape — {mode}")

    scrape_monthly_revenue(db)
    scrape_financial(db)

    # Log completion
    db["scrape_log"].insert_one({
        "timestamp": datetime.now(),
        "mode": "ALL" if SAVE_ALL else "FILTERED",
        "stock_count": "ALL" if SAVE_ALL else len(STOCK_IDS),
        "status": "completed",
    })
...
def main():
    ...
    if "--watch" in sys.argv:
        ...
        local_hour = (SCHEDULE_HOUR + TZ_OFFSET) % 24
        log.info(f"Watch mode: run now, then daily at {SCHEDULE_HOUR:02d}:00 UTC ({local_hour:02d}:00 UTC+{TZ_OFFSET})")
        run_all(db)
