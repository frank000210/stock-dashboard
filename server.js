...
function getTWYear() { return new Date().getFullYear() - 1911; }
    ...
    for (let i = 0; i < 15 && rows.length < 13; i++) {
      ...
      try {
        const resp = await axios.post(
          ...
          { headers: { ...BASE_HEADERS, 'Content-Type': 'application/json' }, timeout: 12000 }
        );
        const result = resp.data && resp.data.result;
        if (resp.data && resp.data.code === 200 && result && result.data) {
          const yymm = result.yymm || '';
          const monthRow = result.data.find(function(r) { return r[0] === '本月'; });
          const cumRow = result.data.find(function(r) { return r[0] === '累計'; });
          const yoyRow = result.data.find(function(r) { return r[0] && r[0].includes('去年同期'); });
    ...
    for (let back = 0; back < 90 && results.length < 30; back++) {
      ...
      try {
        const resp = await axios.get(
          ...
          { headers: { ...BASE_HEADERS, 'Referer': 'https://www.twse.com.tw/zh/fund/TWT38U' }, timeout: 10000 }
        );
        if (resp.data && resp.data.stat === 'OK' && resp.data.data && resp.data.data.length > 0) {
...
app.get('/api/financial/:stockId', async (req, res) => {
  ...
  try {
    ...
    for (let pi = 0; pi < periods.length; pi++) {
      ...
      const year = periods[pi].year;
      const season = periods[pi].season;
      try {
        const resp = await axios.post(
          'https://mops.twse.com.tw/mops/api/t164sb04',
          { companyId: stockId, dataType: '2', year: String(year), season: String(season).padStart(2,'0'), subsidiaryCompanyId: '' },
          { headers: { ...BASE_HEADERS, 'Content-Type': 'application/json' }, timeout: 15000 }
        );
        const result = resp.data && resp.data.result;
        if (resp.data && resp.data.code === 200 && result && result.reportList && result.reportList.length > 0) {
          ...
          if (revenue > 0 || eps !== 0) {
            results.push({ period: (year+1911) + qMap[season], revenue, opIncome, netIncome, eps });
          }
        }
      } catch (e) {}
    }
  ...
  try {
    ...
    for (let back = 0; back < 5 && !result; back++) {
      ...
      try {
        ...
        const dd = resp.data;
        if (dd && dd.stat === 'OK' && dd.data && dd.data.length > 0) {
